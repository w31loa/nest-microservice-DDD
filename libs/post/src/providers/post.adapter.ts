...
export class PostAdapter implements PostRepository{
    ...
    async findAll(pagination: PaginationDto): Promise<[PostAggregate[], number]> {
        ...
        const { limit: take , offset: skip } = plainToInstance(PaginationDto, pagination) // создает инстанс класса с дефолтными значениями или теми которые были переданны

        const options:FindManyOptions<PostEntity> = { // это крутая штука используем тип чтобы были подсказки опций тайп орм и дженерик чтобы были подсказки сущности
            where:{
                isPublished: true
            },
            take,
