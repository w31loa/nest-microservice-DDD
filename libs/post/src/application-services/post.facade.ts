...
import { CreatePostCommand, CreatePostCommandHandler, DeletePostCommand, DeletePostCommandHandler, SetPublishedCommand, SetPublishedCommandHandler, UpdatePostCommand, UpdatePostCommandHandler } from "./commands";
import { GetPostQuery, GetPostQueryHandler, GetPostsQuery, GetPostsQueryHandler } from "./queries";
import { PaginationDto } from "@lib/shared/dto";

@Injectable()
export class PostFacade { //cqrs
    constructor(
    ...
    queries = {
        getOnePosts: (id:string) => this.getPost(id),
    ...
    private createPost(post:CreatePostDto){
        // при помощи такого страшного дженерика мы можем добиться чтобы возвращаемый тип был PostAggrigate обратившись к хендлеру а точнее к его методу execute
